...
export class FacturacionComponent implements OnInit {
  ...
  aplicarFiltrosFacturadas() {
    if (this.periodoFacturadas === 'CUSTOM' && (!this.fechaInicioFacturadas || !this.fechaFinFacturadas)) {
      this.messageService.add({ severity: 'warn', summary: 'Validación', detail: 'Selecciona fecha inicial y final para el rango personalizado.', life: 3500 });
      return;
    }
  ...
  abrirDialogoFacturacionConsolidada() {
    ...
    if (hayTipoVentaDistinto) {
      this.messageService.add({ severity: 'warn', summary: 'Tipos de venta distintos', detail: 'La factura consolidada solo admite ventas del mismo tipo (ej. solo Contado o solo Crédito). No mezclar.', life: 5500 });
      return;
    }
